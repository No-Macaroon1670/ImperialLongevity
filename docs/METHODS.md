...
- **建议文档办结即清**（2026-08-25 用户令）：凡审核建议（「值得」「必加」类）已办结者
  （落库或正式判不立），回源把建议文档里的触发句改写成「已裁/已立条」注记并去掉触发词，
  下次拉网不再重复命中。拉网卫生两条随立：①拉网只扫 .md 建议文——工作流 .json 档案
  属 prompt/判词记录，不是建议，不入网；②「不值得」等负面判词不算建议命中。
