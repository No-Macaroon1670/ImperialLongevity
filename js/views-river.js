...
// 这一张把时间竖过来、把河宽整个交给「当时并存的政权」瓜分，读的是**分合岔流**——
...
export function renderRiver(host, list, opts) {
  ...
  // 轨上那两种印记的口径归滑杆自己（2026-09-08 §一.3）：从前写在图例散文的
  // 第五句上，而读者是在**摸到滑杆**的时候才问「这些色段是什么」
  scrub.title = '纪年滑杆：拖动即跳到那一年。轨上的色段＝天下一统的时段，短横＝时代分界';
  scrub.setAttribute('aria-label', scrub.title);
  ...
  // ── 图例行 ──────────────────────────────────────────────────────────────
  // 与泳道同规格（2026-09-08 去 clutter 案 D1）：一词一义、按在场要素显隐、「·」分隔。
  // 从前这里是 355 字散文 ＋ 一段事件说明 ＋ 一块 2583 字的「这张图为什么这样画」。
  // 铁律是「为什么这样画」零正文：那 12 段已逐条同文在 README.md L144–320，
  // 读者面的问答体在 docs/desk/about-src-timeline.md（供 about.html 收口）。
  // 留在图旁的只剩读图必需的视觉语法——不看会读错的那些词。
  const peak = slices.reduce((m, s) => Math.max(m, s.n), 0);
  const peakSlice = slices.find((s) => s.n === peak);
  // 斜纹（低置信年份）只在图上真有的时候才写进图例：按不动的语法比没有语法更糟
  ...
  if (nhMode === 'all') key.push('河道左缘细彩线＝年号');
  else if (nhMode === 'sel') key.push('点选君主可显年号线');
  // 事件层的读法：宽屏在两岸、窄屏在河道里，两种排法读法不同，说在场的那一种
  if (showEvents) {
    key.push(EV_STRIP > 0
      ? '左岸政事、右岸文教'
      : '事件画进河道；落在河道缝上的＝改朝换代，或归属未定');
  }
  host.appendChild(h('p', { class: 'muted small', style: 'margin:10px 0 0', text: key.join(' · ') }));
  if (showEvents) {
  ...
  // 都直接标着朝代名，颜色只是辅助通道
  host.appendChild(h('p', { class: 'muted small', style: 'margin:8px 0 0' }, [
