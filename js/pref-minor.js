 ...
 *  2026-09-09 库主裁「可以画」之后上了竖河；同日改令「合为一个灰色河流」，
 *  三十六条淡带随即并成**一条**灰河（只占一股河宽，标签写当年并存几家），
 *  故关掉是「少一股河宽」而不再是「少三十六条」；泳道端照旧不画
 *（buildBands 的 meta 带被 renderLaneTimeline 滤掉）。 */
export const MINOR_TIP = `第三层：君主记录不入表的 ${MINOR_KEYS.size} 个政权`
  + '（列国十五、古蜀、卫满朝鲜、于阗、归义军、定难军、五代小藩、土司四家…）；'
  + '关掉后王朝之河不画那条合并的灰河、河宽少一股；横向泳道本就不画；时光舆图不画它们的都城。';
