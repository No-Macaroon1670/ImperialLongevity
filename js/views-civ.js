...
export function renderCiv(host, list, opts) {
  ...
  // 图注只留「怎么读这四个角」；「交班已被制度吸收」那半段推演挪去 about.html
  // （2026-09-08 去 clutter 案 C14）
  host.appendChild(h('p', { class: 'muted small', text:
    '右下＝换得体面（交班已制度化），左上／右上＝篡弑仍是主要退场方式；点面积∝君主数。' }));
  ...
  if (ref) {
    ...
    if (flipped) {
      // 警示条只留一行结论（「以同期对照为准」）；为什么会翻转那段原因收进折叠块
      // ——读者第一眼要知道的是「别信全时段那个数」，理由随时想查（去 clutter 案 C15）
      host.appendChild(h('div', { class: 'notice warn' }, [
        h('strong', { text: `⚠ ${flipped} 组比较在同期对照下翻转——以同期为准。` }),
      ]));
      host.appendChild(notes([
        '中国横跨两千一百年，而其余文明各自集中在某几个世纪；中国自身的非正常死亡率也随时代显著下降。'
        + '凡标注「翻转」者，全时段的差距很大程度上是「近世普遍比上古安全」，而非政治结构之别。',
      ], { label: '为什么会在同期对照下翻转' }));
    }
  }
