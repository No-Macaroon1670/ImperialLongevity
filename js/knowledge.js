...
const NOTABLE = new Map(Object.entries({
  // 先秦（键是 e.name 实串——列国多为「芈侣/熊侣」式双写；夏商段低置信，
  // 仍按「任意百年视窗至少一位在榜」补满：槐、不降这类冷王是覆盖用的填充位）
  姒文命: 3, 姒太康: 1, 姒少康: 1, 姒槐: 1, 姒不降: 1, 姒孔甲: 1, 姒履癸: 2,
  子履: 3, 子密: 2, 子滕: 1, 子旬: 2, 子昭: 3, 子瞿: 1, 子受: 3,
  姬发: 3, 姬满: 2, 姬胡: 2, 姬静: 1, 姬宫湦: 2,
  姬宜臼: 2, 姬匄: 1, 姬延: 2,
  寒浞: 1, 姬昌: 3, 共伯和: 1,
  姜小白: 3, 姬重耳: 3, '芈通/熊通': 1, '芈侣/熊侣': 3, 嬴任好: 2,
  姬光: 2, 姒勾践: 3, 魏斯: 2, 嬴渠梁: 2, 田因齐: 2, 嬴稷: 2, 赵雍: 3, 姬职: 2,
  // 秦汉
  嬴政: 3, 胡亥: 2, 陈胜: 2, 项羽: 3, 刘邦: 3, 刘恒: 2, 刘启: 2, 刘彻: 3, 刘弗陵: 1, 刘询: 2, 刘骜: 1,
...
const DYN_WIKI = {
  ...
  // 单主客串政权（逐一 REST 核验；大燕/大齐裸名会落到别处，岐周落先周条目）
  有穷: '有穷氏', 岐周: '先周', 共国: '共国', 张楚: '张楚政权', 西楚: '西楚',
  大燕: '燕 (安史之乱)', 大齐: '齐 (黄巢)', 陈汉: '陈汉', 大顺: '大顺 (政权)',
  // 胡夏裸名在维基的正题是 1990 年生的男歌手（standard 非消歧义，
  // 连 alt 退路都不触发——用户两度撞见）。十六国那个在「夏 (十六国)」
  胡夏: '夏 (十六国)',
  ...
  南燕: '南燕 (十六國)',
  // 小政权第一批（2026-09-04 卷 xiaozq 实测）：殷／燕／赵裸名是消歧义页，北平重定向到民国北平市，
  // 中天八国重定向到人物条张遇贤——五条写死，其余（于阗、归义军、岐、李蜀、蒙兀国）裸名落对不入表
  殷: '殷 (五代十国)', 燕: '燕 (五代十国)', 赵: '赵 (五代十国)', 北平: '北平 (五代十国)', 中天八国: '张遇贤',
  // 小政权第二批（2026-09-04 卷 xiaozq2 实测）：水东裸名是消歧义页；播州／思州裸名落在历史州条，政权层挂土司条更对题；
  // 定难军／武平军裸名重定向到节度使条、长其／自杞／东夏／水西裸名落对，皆不入表
  播州: '播州土司', 思州: '思州土司', 水东: '水东土司',
  // 小政权（2026-09-09 卷 D01）：「辽东公孙」「公孙氏政权」「公孫氏燕國」「燕 (三國)」裸名皆无正题，
  // 《遼東公孫氏》才是维基那一条（非重定向非消歧义）；「燕」在本表已归五代十国那个
  公孙氏辽东: '遼東公孫氏',
};
const DYN_BAIDU = {
  ...DYN_WIKI,
  梁: '南朝梁', 陈: '南朝陈', 楚: '南楚', 胡夏: '赫连夏', 中华帝国: '洪宪帝制',
  // 先秦（P5 同源实测，义项号从「战国七雄」「春秋五霸」词条 DOM 收割并逐一验明）：
  // 裸名劫持三例——晋国→西晋、齐国→北齐、吴国→孙吴；韩国裸发落大韩民国。
  // 楚国/秦国/燕国/赵国/魏国/越国/田齐/夏朝/商朝/西周/东周 裸名实测落对，不入表。
  晋国: '晋国/4862635', 姜齐: '齐国/50284', 吴国: '吴国/13313', 韩国: '韩国/6009300',
  宋国: '宋国/31041', 郑国: '郑国/3680',   // 郑国裸名在百度落「郑国世系」同号条目
  // 单主客串政权（义项号从各君主词条 DOM 收割：陈胜→张楚政权、项羽→西楚、
  // 黄巢→大齐、李自成→大顺政权、陈友谅→陈汉、寒浞→有穷氏、姬昌→岐周、共伯和→共国）。
  // 大燕裸名是安徽一个已废止的地名，百度并无安史燕政权的词条（伪燕 404）——
  // 按「不强凑」的规矩指到安史之乱，讲的正是这个政权的始末
  有穷: '有穷氏/447037', 岐周: '岐周/9633908', 共国: '共国/471531',
  张楚: '张楚政权/9396465', 西楚: '西楚/3745853', 大燕: '安史之乱',
  大齐: '大齐/8068172', 陈汉: '陈汉/15285334', 大顺: '大顺政权/3877690',
  // 小政权（2026-09-09 卷 D01 复核补）：本表继承 DYN_WIKI 会拿到繁体正题「遼東公孫氏」，
  // 百度对它与简体「辽东公孙氏」两发皆 302→error.html?status=404（本轮 curl 实测）；
  // 百度自己的词条叫「公孙氏政权」（200，导言「东汉末年至三国时期割据辽东的地方势力」），故写死覆盖
  公孙氏辽东: '公孙氏政权',
};
...
const empSpec = (item) => {
  ...
  return {
    ...
    // 百度侧分层（P5 实测）：夏王用姒名（姒文命→禹 落对，通称「太康」会撞太康县）；
    // 商周与列国用通称（帝辛/秦昭襄王/晋文侯 皆实测落对——人名「子受」撞漫画角色、
    // 「嬴稷」是多义词、「芈侣/熊侣」带斜杠会被读成义项语法）；帝制时代照旧人名
