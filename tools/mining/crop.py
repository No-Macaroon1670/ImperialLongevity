...
五层文件夹（2026-08-26 用户改定「出品留原片」，加第五夹；原四夹制 2026-08-21 定）：
  ...
  img/originals/   原片留档（**本地，不进 git**）：凡出品（进 own/ 或 used/）的照片，
                   其原片改名 `<成品名>-orig.jpg` 移入此夹**永久保留**，EXIF 不剥
                   （本夹永不上公网）。立规缘起：2026-08-26 审计发现 880→1600 换 spec 时
                   15 张 used 因「完工即清」无源可重制，只能请用户翻相册重供——
                   spec 会再变（分辨率、裁法、打码标准都可能），原片在手就地重做，
                   翻相册考古只此一回。
...
管线规矩（越攒越多，都是实测立下的）：
  ...
  · **带说明牌而明显歪斜的照片：先转正（旋转校正）再裁**（2026-08-26 用户加行）——
    牌是据，据要读得正；斜拍的牌先旋到水平再定裁框，别裁完才发现字是斜的。
  ...
  · **五夹流转，开工即移，出品留原片，落选才清，上站才晋升**（2026-08-26 用户
    改定，废「完工即清一刀切」；原四夹流转 2026-08-21 立，实案：元旦国博组判毕
    未移出 inbox，隔批整组被重复吃工）：inbox 是用户放料区；一开始处理就把该批
    移入 processing（inbox 只剩未开工者，文件数才配叫欠账数）；完工时成品入
    own/，**出品的原片改名 `<成品名>-orig.jpg` 移 img/originals/ 永久留档**；
    落选照片与中间产物照旧清走。旧规「原图正本在用户相册里，副本不必囤」
    只对落选片继续成立——出品片的原片就是将来换 spec 重做的本钱，不囤它，
    重做就得翻相册考古（2026-08-26 十五张缺源实案）。
    〔回溯：立规当日已将 processing 存世的 37 张成品名 -orig 移入 originals/，
    惜皆 957 档中间产物；此前批次的全尺寸原片已按旧规清灭，缺源单在
    desk/photo-resupply-15-20260826.md。〕
    ...
    **三条护栏（2026-08-20 knock-on 审计补）**：①「完工」按张计不按批计——
    ...
    清仓后重裁须请用户从相册重供（2026-08-26 起出品母图原片永存 originals/，
    此护栏只余落选片一段仍然要紧）；③凡被候选底稿/账册引为证据的说明牌照
